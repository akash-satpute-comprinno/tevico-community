...
class CodebaseContextProvider:
    ...
    def _run_probe(self, query: str, language: str = None, max_tokens: int = 2000) -> str:
        """Run a probe search query and return results"""
        if not self.probe_available:
            return ""
        try:
            cmd = ["probe", "search", query, self.repo_path, f"--max-tokens={max_tokens}"]
            if language:
                cmd += [f"--lang={language}"]
    ...
    def get_context_for_file(self, file_path: str, code: str, language: str) -> str:
        ...
        # Extract key terms from the file path and code to search for
        ...
        # Search 1 — Find similar patterns in the codebase
        for term in search_terms[:3]:
            results = self._run_probe(term, language=language, max_tokens=800)
            if results and file_path not in results:
                context_parts.append(f"### Existing '{term}' patterns:\n{results}")

        # Search 2 — Find error handling conventions
        error_results = self._run_probe("exception OR error handling", language=language, max_tokens=600)
        if error_results:
            context_parts.append(f"### Error handling conventions:\n{error_results}")

        # Search 3 — Find logging conventions
        log_results = self._run_probe("logger OR logging", language=language, max_tokens=400)
        if log_results:
            context_parts.append(f"### Logging conventions:\n{log_results}")

        if not context_parts:
            return ""

        return "## Codebase Context — How similar things are done in this project:\n\n" + \
               "\n\n".join(context_parts[:4])  # cap at 4 sections
