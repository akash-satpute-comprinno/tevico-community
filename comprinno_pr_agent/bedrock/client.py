...
{{"status": "resolved|still_present|partial", "reason": "brief explanation"}}"""
    ...
    def find_new_issues(self, code: str, language: str, file_path: str,
                        known_issues: list = None, ticket_info: dict = None) -> Dict[str, Any]:
        """Find only NEW issues — skip anything already in known_issues"""
        known_section = ""
        if known_issues:
            items = "\n".join(f"  - [{f['category']}] Line {f['line']}: {f['description'][:100]}" for f in known_issues)
            known_section = f"""## ALREADY KNOWN ISSUES (DO NOT REPORT THESE)
{items}

Only report issues NOT in the above list.
...
Based on the full ticket, evaluate what's done, partial, and not done.
Return in "ticket_completion" field.

"""

        prompt = f"""You are an expert code reviewer. Find NEW issues only in this {language} code.

{known_section}{ticket_section}
File: {file_path}
...
{{
  ...
  "ticket_completion": {{
    "done": ["requirement — verified correct: explanation"],
    "not_done": ["requirement — what needs to be implemented"],
    "partial": ["requirement — what was done vs what's still missing"]
  }}
}}
